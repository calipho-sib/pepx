...
char *code4tenAA(char *currentAC)
{
    ...
    static char *tab[] = {"A A0A024", "B A0A044", "C A0A067", "D A0A075", "E A0A087", "F A0A088", "G A0A096", "H A0A0A0", "I A0A0A6", "J A0A0B4", "K A0A0C3",
                          ...
                          "j A0A2R8", "k A0A0K2", "l A0A2Z4", "m A0A494", "n A0A5F9"
                         };
...
int pepx_create_variant_files(char *varfolder)
{
    ...
    sprintf(fname, "%s/VARIANTS_OK", varfolder); // flag file
    out = fopen(fname, "w");
    fclose(out);
    return(1);
}
...
int pepx_build_varindex(char *isoname)
{
    ...
    // Get variants from NextProt files
    ...
    while(fgets(buf, LINELEN, NextProtvariants))
    {
        *varAA = 0; //To avoid that last value is keeped when scaning a miss variant (varAA empty)
        ...
        if(fpos == 1 || strchr(varAA, '*') || (strlen(orgAA) > 2)  || (strlen(varAA) > 2))
            // No variants on Met-1 allowed, No stop variants allowed, No long variants
        {
            ...
            if(bsearch(newquery, NULL, idx[pepsize].elemcnt, pepsize + INDEX2WIDTH, pepcompare))
            {
                    ...
                    // get all ACs
                {
                    if(strlen(ac) > MAXPEPSIZE + 1) // We're still an accession 
                    {
                        ...
                        if(!found) // TODO: Investigate why it doesn't work 100%, there are still duplicate variants in results when using a joker (eg: HTXEKPY)
...
void pepx_build(char *seqfilename)
{
    ...
    if(!ignore_variants)
    {
        fprintf(stdout, "Indexing sequences with variants, please wait...\n");
        ...
        if((varcheck = fopen(fname, "r")) == NULL)
            // flag file absent: we must build the files
        {
    ...
    {
        for(i = 0; buf[0] == '#'; i++)
            // PEFF file starts with #-commented description lines
    ...
    while(fgets(buf, MAXHEADSIZE, in))
    {
        ...
        if(FASTAinput) // We're parsing the PEFF file
            pepx_getFASTAentry(currISO, buf, in, &PEFFvarcnt); // TODO: implement PEFFvarcnt
        else // Parsing from prerelease data
        {
            strncpy(currISO, buf + 3, 12); // get rid of NX_
            if(strchr(currISO, '\t'))
                *strchr(currISO, '\t') = 0;
            if(strchr(currISO, ' '))
                *strchr(currISO, ' ') = 0;
            ...
            // eg: A0A087WTH1 -> PAWTH1
        {
            ...
            if(!strcmp(shortenedAC, "XXXXXX"))
                // A new kind of 10 digit AC has appeared, report
            {
        ...
        if(!ignore_variants)
        {
            if(!FASTAinput)
                ...
                currVarcnt = pepx_build_varindex(currISO); // temporary
